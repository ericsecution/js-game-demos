...
window.onload = function () {
  var gameData = require('./game_map.json');
  scene('game', function () {
    var rooms = JSON.parse(JSON.stringify(gameData));
    var roomDetails = rooms[0];
  ...
  scene('end', function () {
    add([text('You have finished this Scene!', 6), pos(width() / 2, height() / 2), origin('center')]);
  });
  scene('main', function () {
    // display text
    add([text('Press the Spacebar to Begin!', 6), pos(width() / 2, height() / 2), origin('center')]);
